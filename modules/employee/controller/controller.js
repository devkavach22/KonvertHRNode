...
const createEmployee = async (req, res) => {
  try {
    ...
    try {
      ...
      // MODIFIED: Handle array of approval details
      if (group_id && approval_user_id && approval_sequance !== undefined) {
        try {
          ...
          const userIds = Array.isArray(approval_user_id) ? approval_user_id : [approval_user_id];
          const sequences = Array.isArray(approval_sequance) ? approval_sequance : [approval_sequance];
          const models = Array.isArray(model) ? model : (model ? [model] : []); // Using model instead of type

          // Find the maximum length
          const maxLength = Math.max(groupIds.length, userIds.length, sequences.length, models.length);

          // Loop through and create multiple approval records
          for (let i = 0; i < maxLength; i++) {
            const approvalData = {
              group_id: parseInt(groupIds[i] || groupIds[0]),
              user_id: parseInt(userIds[i] || userIds[0]),
              approval_sequance: parseInt(sequences[i] || sequences[0]),
              employee_id: employeeId,
            };

            // Add model if available
            if (models[i]) {
              approvalData.model = models[i]; // Mapping to 'model' field
            }
      ...
      try {
        ...
        if (employeeData.length > 0) {
          ...
          if (approvalDetails.length > 0) {
            // Mapping multiple records to arrays
            employee.group_id = approvalDetails.map(a => (Array.isArray(a.group_id) ? a.group_id[0] : a.group_id) || "");
            employee.approval_user_id = approvalDetails.map(a => (Array.isArray(a.user_id) ? a.user_id[0] : a.user_id) || "");
            employee.approval_sequance = approvalDetails.map(a => a.approval_sequance || "");
            employee.model = approvalDetails.map(a => a.model || "");
          } else {
            employee.group_id = [];
...
const getEmployeeById = async (req, res) => {
  try {
    ...
    // Fetch extra approval details - Added 'model' in fields list
    const approvalDetails = await odooHelpers.searchRead(
      "employee.approval.user.details",
      [["employee_id", "=", employee.id]],
      ["group_id", "user_id", "approval_sequance", "model"] // Added 'model' here
    );

    // Agar multi-approvals hain toh unhe arrays mein format karein
    if (approvalDetails.length > 0) {
      employee.group_id = approvalDetails.map(a => a.group_id[0] || a.group_id);
      employee.approval_user_id = approvalDetails.map(a => a.user_id[0] || a.user_id);
      employee.approval_sequance = approvalDetails.map(a => a.approval_sequance);
      employee.model = approvalDetails.map(a => a.model); // Added this
    } else {
      employee.group_id = [];
      employee.approval_user_id = [];
      employee.approval_sequance = [];
  ...
  try {
    ...
    if (group_id && approval_user_id && approval_sequance !== undefined) {
      try {
        ...
        const userIds = Array.isArray(approval_user_id) ? approval_user_id : [approval_user_id];
        const sequences = Array.isArray(approval_sequance) ? approval_sequance : [approval_sequance];
        const models = Array.isArray(model) ? model : (model ? [model] : []); // Using model
