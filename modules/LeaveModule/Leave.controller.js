...
class LeaveController {
  async createLeaveType(req, res) {
    try {
      ...
      if (
        leave_validation_type &&
        !approvalTypes.includes(leave_validation_type)
      ) {
        return res
          .status(400)
          .json({ status: "error", message: "Invalid leave_validation_type" });
      }

      if (
        allocation_validation_type &&
        !approvalTypes.includes(allocation_validation_type)
      ) {
        return res.status(400).json({
          status: "error",
          message: "Invalid allocation_validation_type",
        });
      }
      ...
      if (requires_allocation && !yesNo.includes(requires_allocation)) {
        return res
          .status(400)
          .json({ status: "error", message: "Invalid requires_allocation" });
      }

      if (employee_requests && !yesNo.includes(employee_requests)) {
        return res
          .status(400)
          .json({ status: "error", message: "Invalid employee_requests" });
      }

      if (leave_category && !categories.includes(leave_category)) {
        return res
          .status(400)
          .json({ status: "error", message: "Invalid leave_category" });
      }

      if (request_unit && !requestUnits.includes(request_unit)) {
        return res
          .status(400)
          .json({ status: "error", message: "Invalid request_unit" });
      }
  ...
  async updateLeaveType(req, res) {
    try {
      ...
      if (leave_validation_type !== undefined)
        vals.leave_validation_type = leave_validation_type;
      if (allocation_validation_type !== undefined)
        vals.allocation_validation_type = allocation_validation_type;
      if (requires_allocation !== undefined)
        vals.requires_allocation = requires_allocation;
      if (employee_requests !== undefined)
        vals.employee_requests = employee_requests;
      ...
      if (include_public_holidays_in_duration !== undefined) {
        vals.include_public_holidays_in_duration = Boolean(
          include_public_holidays_in_duration
        );
      }
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          status: "error",
          message:
            "Required missing fields: holiday_status_id,employee_id,allocation_type,date_from",
        });
      }
      ...
      const leave_type_name =
        leaveTypeInfo.length > 0 ? leaveTypeInfo[0].name : "Unknown Leave Type";
  ...
  async getLeaveAllocation(req, res) {
    try {
      ...
      const { employee_id, leave_type_id, date_from, date_to, status } =
        req.query;
  ...
  async updateLeaveAllocation(req, res) {
    try {
      ...
      if (
        allocation_type &&
        !["regular", "accrual"].includes(allocation_type)
      ) {
        return res.status(400).json({
          status: "error",
          message: "Invalid allocation type",
      ...
      if (employee_id !== undefined) vals.employee_id = parseInt(employee_id);
      ...
      if (description !== undefined) vals.name = description;
      ...
      /* ---------------- UPDATE ---------------- */
      await odooService.write("hr.leave.allocation", parseInt(id), vals);
  ...
  async deleteLeaveAllocation(req, res) {
    try {
      ...
      /* ---------------- CANCEL (SOFT DELETE) ---------------- */
      await odooService.write("hr.leave.allocation", parseInt(id), {
        state: "refuse",
        write_uid: user_id,
      });
    ...
    try {
      ...
      if (!holiday_status_id || !date_from || !date_to) {
        ...
        return res.status(400).json({
          status: "error",
          message:
            "Require fields missing: holiday_status_id, date_from, date_to",
        });
      }
      ...
      console.log(
        `Resolved context: Employee:${empData.name} | Dept:${department_name} | Company:${company_name} | Leave Type:${leave_type_name}`
      );
      ...
      console.log(`Success: Leave Request ID:${requestId} created in Odoo.`);
  ...
  async getLeaveRequest(req, res) {
    try {
      ...
      const { employee_id, leave_type_id, status, date_from, date_to } =
        req.query;
  ...
  async updateLeaveRequest(req, res) {
    try {
      ...
      if (
        state &&
        !["draft", "confirm", "validate", "refuse", "cancel"].includes(state)
      ) {
        return res.status(400).json({
          status: "error",
      ...
      if (reason !== undefined) vals.name = reason;
      ...
      /* ---------------- UPDATE ---------------- */
      await odooService.write("hr.leave", parseInt(id), vals);
  ...
  async deleteLeaveRequest(req, res) {
    try {
      ...
      /* ---------------- CANCEL REQUEST ---------------- */
      await odooService.write("hr.leave", parseInt(id), {
        state: "cancel",
        write_uid: user_id,
      });
...
===================== */
      ...
      if (!adminEmployee.length) throw new Error("Admin employee not found");
...
===================== */
      const normalizeLeave = (l) => ({
        ...
        leave_type:
          l.holiday_status_id?.[1] || l.leave_type || "Unplanned Absence",
...
===================== */
  ...
  async createPublicHoliday(req, res) {
    try {
      ...
      const { name, date_from, date_to, work_entry_type_id, calendar_id } =
        req.body;
      ...
      if (work_entry_type_id) {
        ...
        if (!workEntryType.length) {
          return res
            .status(400)
            .json({ status: "error", message: "Invalid work_entry_type_id" });
        }
      ...
      // 6. Create Holiday
      // -----------------------------
      const holidayId = await odooService.create(
  ...
  async updatePublicHoliday(req, res) {
    try {
       ...
       * 4. Update
       * ----------------------------- */
      await odooService.write("resource.calendar.leaves", parseInt(id), vals);
  ...
  async deletePublicHoliday(req, res) {
    try {
       ...
       * 2. Hard Delete
       * ----------------------------- */
      await odooService.unlink("resource.calendar.leaves", parseInt(id));
  ...
  async updateAccrualPlan(req, res) {
    try {
      ...
      if (accrued_gain_time !== undefined)
        vals.accrued_gain_time = accrued_gain_time;
       ...
       * 5. Update Record
       * ----------------------------- */
      await odooService.write("hr.leave.accrual.plan", parseInt(id), vals);
  ...
  async deleteAccrualPlan(req, res) {
    try {
       ...
       * 2. Hard Delete
       * ----------------------------- */
      await odooService.unlink("hr.leave.accrual.plan", parseInt(id));
  ...
  async updateMandatoryDays(req, res) {
    try {
       ...
       * 5. Update Record
       * ----------------------------- */
      await odooService.write("hr.leave.mandatory.day", parseInt(id), vals);
  ...
  async deleteMandatoryDays(req, res) {
    try {
       ...
       * 2. Hard Delete
       * ----------------------------- */
      await odooService.unlink("hr.leave.mandatory.day", parseInt(id));
